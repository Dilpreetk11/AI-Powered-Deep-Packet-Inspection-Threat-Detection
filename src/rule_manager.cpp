...
std::optional<RuleManager::BlockReason> RuleManager::shouldBlock(
    ...
    // Check AI Threat Threshold
    if (threat_score > 0 && threat_score >= ai_threat_threshold_) {
        return BlockReason{BlockReason::AI_THREAT, "Threat Score: " + std::to_string(threat_score) + "/100"};
    }
