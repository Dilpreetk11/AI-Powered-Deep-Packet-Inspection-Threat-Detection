...
DPIEngine::DPIEngine(const Config& config)
    ...
    std::cout << "║   AI Engine:         " << (config.ai_enabled ? "ENABLED " : "DISABLED") << "                                       ║\n";
    std::cout << "║   AI Threat Thresh:  " << std::setw(3) << config.ai_threat_threshold << "/100                                   ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n";
}
...
bool DPIEngine::initialize() {
    ...
    rule_manager_->setAIThreatThreshold(config_.ai_threat_threshold);
    ...
    AIConfig ai_cfg;
    ai_cfg.enabled = config_.ai_enabled;
    ai_cfg.threat_threshold = config_.ai_threat_threshold;
    ai_cfg.classifier_model_path = config_.ai_classifier_model;
    ai_cfg.anomaly_model_path = config_.ai_anomaly_model;
    ai_cfg.verbose = config_.verbose;
    ...
    fp_manager_ = std::make_unique<FPManager>(total_fps, rule_manager_.get(), output_cb, ai_engine_.get());
    ...
    std::cout << "[DPIEngine] Initialized successfully with AI Subsystem\n";
    return true;
}
...
std::string DPIEngine::generateReport() const {
    ...
    if (ai_engine_) {
        auto ai_stats = ai_engine_->getStats();
        ss << "╠══════════════════════════════════════════════════════════════╣\n";
        ss << "║ AI THREAT INTELLIGENCE & CLASSIFICATION                       ║\n";
        ss << "║   Evaluated Flows:    " << std::setw(12) << ai_stats.total_evaluations << "                        ║\n";
        ss << "║   High Threats (>=70):" << std::setw(12) << ai_stats.high_threat_count << "                        ║\n";
        ss << "║   Medium (40-69):     " << std::setw(12) << ai_stats.medium_threat_count << "                        ║\n";
        ss << "║   Low (0-39):         " << std::setw(12) << ai_stats.low_threat_count << "                        ║\n";
    }
    ...
    if (rule_manager_) {
        ...
        ss << "║   AI Threat Thresh:   " << std::setw(11) << rule_manager_->getAIThreatThreshold() << "/100                        ║\n";
    }
