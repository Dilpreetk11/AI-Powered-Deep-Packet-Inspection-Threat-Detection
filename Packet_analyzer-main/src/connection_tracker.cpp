...
void ConnectionTracker::updateConnection(Connection* conn, size_t packet_size, bool is_outbound) {
    updateConnection(conn, packet_size, is_outbound, 0, -1.0);
}

void ConnectionTracker::updateConnection(Connection* conn, size_t packet_size, bool is_outbound, uint8_t tcp_flags, double payload_entropy) {
    if (!conn) return;
    
    auto now = std::chrono::steady_clock::now();
    if (conn->pkt_lengths.size() > 0) {
        float iat = std::chrono::duration<float>(now - conn->last_seen).count();
        if (conn->pkt_iats.size() < 16) {
            conn->pkt_iats.push_back(iat);
        }
    }
    conn->last_seen = now;
    ...
    if (conn->pkt_lengths.size() < 16) {
        float len_signed = is_outbound ? static_cast<float>(packet_size) : -static_cast<float>(packet_size);
        conn->pkt_lengths.push_back(len_signed);
    }

    if (payload_entropy >= 0.0) {
        conn->total_payload_entropy += payload_entropy;
        conn->payload_entropy_count++;
    }

    if (tcp_flags & 0x02) conn->syn_count++; // SYN
    if (tcp_flags & 0x01) conn->fin_count++; // FIN
    if (tcp_flags & 0x04) conn->rst_count++; // RST
}
