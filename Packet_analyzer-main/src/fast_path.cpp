...
PacketAction FastPathProcessor::processPacket(PacketJob& job) {
    ...
    double payload_entropy = -1.0;
    if (job.payload_length > 0 && job.payload_offset < job.data.size() && ai_engine_) {
        const uint8_t* payload = job.data.data() + job.payload_offset;
        payload_entropy = ai_engine_->calculatePayloadEntropy(payload, job.payload_length);
    }

    // Update connection stats & sequence features
    bool is_outbound = true;  // In this model, all packets from user are outbound
    conn_tracker_.updateConnection(conn, job.data.size(), is_outbound, job.tcp_flags, payload_entropy);
    ...
    // Trigger AI evaluation asynchronously/once per flow when minimum packet window reached
    if (ai_engine_ && (!conn->ai_result || !conn->ai_result->evaluated)) {
        if (conn->pkt_lengths.size() >= 5 || conn->state == ConnectionState::CLASSIFIED) {
            conn->ai_result = ai_engine_->evaluateFlow(*conn);
            if (conn->ai_result->predicted_app != AppType::UNKNOWN && conn->app_type == AppType::UNKNOWN) {
                conn->app_type = conn->ai_result->predicted_app;
            }
        }
    }
    
    // Check rules (including AI Threat Thresholds)
...
PacketAction FastPathProcessor::checkRules(const PacketJob& job, Connection* conn) {
    ...
    uint32_t threat_score = (conn->ai_result) ? conn->ai_result->threat_score : 0;
    ...
    if (block_reason) {
        ...
        switch (block_reason->type) {
            ...
            case RuleManager::BlockReason::PORT:
                ...
                break;
            case RuleManager::BlockReason::AI_THREAT:
                ss << "AI Threat " << block_reason->detail;
                if (conn->ai_result) {
                    ss << " (" << conn->ai_result->rationale << ")";
                }
                break;
        }
...
FPManager::FPManager(int num_fps,
    ...
    for (int i = 0; i < num_fps; i++) {
        auto fp = std::make_unique<FastPathProcessor>(i, rule_manager, output_callback, ai_engine);
