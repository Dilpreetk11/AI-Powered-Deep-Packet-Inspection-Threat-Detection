...
class ThreatScorer:
    ...
    def score(features, pred_app, app_conf, anomaly_score, attack_type, app_xai, anomaly_xai, sni):
        ...
        decision = "BLOCK" if (final_score >= 70 or (attack_type != "BENIGN" and anomaly_score > 0.50)) else "ALLOW"
...
def run_dpi(input_file, output_file, block_apps=None, block_ips=None, block_domains=None, threat_threshold=70):
    ...
    while True:
        ...
        if not fl['blocked']:
            ...
            elif fl['ai_result'] and (fl['ai_result']['threat_score'] >= threat_threshold or fl['ai_result']['decision'] == 'BLOCK'):
                fl['blocked'] = True
...
if __name__ == '__main__':
    ...
    if len(args) >= 2:
        ...
        i = 2
        while i < len(args):
            if args[i] == '--block-app' and i + 1 < len(args):
                block_apps.append(args[i+1]); i += 2
            elif args[i] == '--block-ip' and i + 1 < len(args):
                block_ips.append(args[i+1]); i += 2
            elif args[i] == '--block-domain' and i + 1 < len(args):
