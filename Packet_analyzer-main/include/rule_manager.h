...
public:
    ...
    // ========== AI Threat Threshold Blocking ==========
    void setAIThreatThreshold(uint32_t threshold) { ai_threat_threshold_ = threshold; }
    uint32_t getAIThreatThreshold() const { return ai_threat_threshold_; }
    ...
    struct BlockReason {
        enum Type { IP, APP, DOMAIN, PORT, AI_THREAT } type;
        std::string detail;
    };
...
private:
    ...
    uint32_t ai_threat_threshold_ = 70;
